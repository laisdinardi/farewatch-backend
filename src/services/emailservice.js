...
function buildEmailHTML(\{ route, priceData, analysis, userName \}) \{\
  const discountLabel = analysis.is50Deal ? '\uc0\u55357 \u56613  50%+ OFF' : '\u55357 \u56521  20%+ OFF';\
  const bgColor = analysis.is50Deal ? '#dc2626' : '#2563eb';\
  const discountBadge = analysis.is50Deal ? '\uc0\u55357 \u56613  MEGA DEAL' : '\u55357 \u56521  GREAT DEAL';\
\
  return `\
<!DOCTYPE html>\
<html>\
...
\
    ...
    <div style="padding:32px; border-bottom:1px solid #f0f0f0;">\
      <div style="text-align:center; margin-bottom:24px;">\
        ...
        <span style="font-size:20px; color:#999; margin:0 16px;">\uc0\u8594 </span>\
      ...
      $\{route.travel_date ? `<div style="text-align:center; color:#666; font-size:13px; margin-top:4px;">\uc0\u55357 \u56517  $\{new Date(route.travel_date + 'T00:00:00').toLocaleDateString('pt-BR', \{ weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' \})\}</div>` : ''\}\
      $\{route.flexible_month ? `<div style="text-align:center; color:#666; font-size:13px; margin-top:4px;">\uc0\u55357 \u56517  $\{new Date(route.flexible_month + '-01').toLocaleDateString('pt-BR', \{ year: 'numeric', month: 'long' \})\}</div>` : ''\}\
    </div>\
\
    <!-- Price Comparison -->\
...
\
    ...
    <div style="padding:24px 32px; border-bottom:1px solid #f0f0f0;">\
      ...
      <div style="margin-bottom:8px; font-size:14px; color:#666;">\
        \uc0\u9992 \u65039  <strong>Airline:</strong> $\{priceData.airline\}\
      </div>` : ''\}\
      $\{priceData.stops !== undefined ? `\
      <div style="margin-bottom:8px; font-size:14px; color:#666;">\
        \uc0\u55357 \u57041  <strong>Stops:</strong> $\{priceData.stops === 0 ? 'Direct flight' : `$\{priceData.stops\} stop(s)`\}\
      </div>` : ''\}\
      $\{priceData.duration_minutes ? `\
      <div style="margin-bottom:8px; font-size:14px; color:#666;">\
        \uc0\u9201 \u65039  <strong>Duration:</strong> $\{Math.floor(priceData.duration_minutes / 60)\}h $\{priceData.duration_minutes % 60\}m\
      </div>` : ''\}\
    </div>\
\
    <!-- CTA -->\
...
\
  try \{\
    ...
    const discountLabel = analysis.is50Deal ? '\uc0\u55357 \u56613  50%+ DEAL' : '\u55357 \u56521  20%+ DEAL';\
\
    await mailer.sendMail(\{\
      from: process.env.EMAIL_FROM || `"FareWatch" <$\{process.env.SMTP_USER\}>`,\
      to: user.email,\
      subject: `$\{discountLabel\}: $\{route.origin_iata\} \uc0\u8594  $\{route.destination_iata\} por $\{new Intl.NumberFormat('pt-BR', \{ style: 'currency', currency: 'BRL' \}).format(priceData.price_brl)\}`,\
