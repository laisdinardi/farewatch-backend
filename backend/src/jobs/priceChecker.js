...
\f0\fs24 \cf0 const \{ supabase \} = require('../../config/supabase');\
...
\
  ...
  logger.info('\uc0\u55357 \u56589  Starting price check job...');\
\
  try \{\
...
\
    ...
    logger.info(`\uc0\u9989  Price check completed in $\{duration\}s. Deals: $\{dealsFound\}, Alerts: $\{alertsSent\}`);\
  \} finally \{\
    isRunning = false;\
  \}\
\}\
...
async function processRoute(route, user) \{\
  logger.debug(`Checking: $\{route.origin_iata\} \uc0\u8594  $\{route.destination_iata\}`, \{\
...
\
  ...
  if (analysis.is50Deal || analysis.is20Deal) \{\
    logger.info(`\uc0\u55356 \u57263  DEAL DETECTED: $\{route.origin_iata\}-$\{route.destination_iata\} $\{analysis.discountPercent\}% off`, \{\
