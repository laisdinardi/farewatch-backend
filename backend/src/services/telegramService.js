...
function setupBotHandlers(bot) {
  ...
  bot.onText(/\/status/, async (msg) => {
    ...
    if (user) {
      bot.sendMessage(msg.chat.id,
        `✅ *Account linked!*\n\n👤 ${user.name || user.email}\n📧 ${user.email}\n🔔 Notifications: ${user.notify_telegram ? 'Enabled' : 'Disabled'}`,
        { parse_mode: 'Markdown' }
      );
    } else {
      bot.sendMessage(msg.chat.id,
  ...
  bot.onText(/\/stop/, async (msg) => {
    ...
    bot.sendMessage(msg.chat.id, '🔕 Notifications paused. Use /start to re-enable.');
  });
...
async function handleLinkAccount(bot, chatId, token, msg) {
  try {
    ...
    if (error || !linkData) {
      return bot.sendMessage(chatId, '❌ Invalid link. Please generate a new one from FareWatch settings.');
    }

    if (linkData.used) {
      return bot.sendMessage(chatId, '❌ This link has already been used. Generate a new one.');
    }

    if (new Date(linkData.expires_at) < new Date()) {
      return bot.sendMessage(chatId, '❌ This link has expired. Generate a new one from settings.');
    }
...
async function sendTelegramAlert({ chatId, route, priceData, analysis }) {
  ...
  try {
    const discountEmoji = analysis.is50Deal ? '🔥' : '📉';
    ...
    const message = [
      ...
      '',
      `✈️ *${route.origin_iata} → ${route.destination_iata}*`,
      `${route.origin_city || route.origin_iata} → ${route.destination_city || route.destination_iata}`,
      '',
      `💰 *Price: ${formatBRL(priceData.price_brl)}*`,
      `📊 Baseline: ${formatBRL(analysis.baseline)}`,
      `💸 You save: ${formatBRL(analysis.savingsAmount)} (${Math.round(analysis.discountPercent)}% off)`,
      '',
      priceData.airline && priceData.airline !== 'Unknown' ? `🏷️ ${priceData.airline} · ${stopsText}` : '',
      route.travel_date ? `📅 ${new Date(route.travel_date + 'T00:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' })}` : '',
      route.flexible_month ? `📅 ${new Date(route.flexible_month + '-01').toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })}` : '',
      '',
      `[🎫 Book Now](${priceData.booking_link || 'https://www.kiwi.com'})`,
    ].filter(Boolean).join('\n');
